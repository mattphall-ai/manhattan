...
| `BenchmarkTactic` | `benchmarkLibrary.ts` | Reusable estimate template (name, category, description, role hours, OOP item) that any org member can browse and load as a new `Project`. See §2a. |
...
## 2a. Benchmark Library

`src/benchmarkLibrary.ts` currently ships 57 benchmark tactics (Animation, Edit & Audio, XR, Digital Art, Live & Virtual Events) as a static, hardcoded array bundled into the client. In the backend, this becomes an org-scoped, centrally-managed table instead of a file every user redeploys to change:

- **Read access**: any authenticated org member can list/browse tactics (`BenchmarkCategory`, name, description, OOP item, `roleHours: { roleId, hours }[]`) and load one into a new `Project` — this is exactly today's "Load Benchmark" flow in `ProjectSelector`/`BenchmarkLibrary.tsx`, just backed by the API instead of a bundled constant.
- **Write access**: Admin-only. Adding/editing tactics (e.g. importing a refreshed benchmark spreadsheet each cycle) should not require a code deploy.
- **Source data discipline**: the tactic data is derived from internal benchmark spreadsheets that, in their raw form, contain named-employee rows and individual billing rates. Only the role/title-level aggregate (no employee names, no individual rates) is ever imported into `BenchmarkTactic` — this rule carries forward from the frontend import and applies equally to whatever admin tool replaces manual spreadsheet parsing on the backend.
- **Versioning**: tactics get refreshed periodically (e.g. a new benchmark cycle). Loading a tactic into a `Project` should copy its data at that point in time — a `Project` must never live-reference a `BenchmarkTactic` that could later change underneath it.
...
- `GET /benchmark-tactics`, `GET /benchmark-tactics/:id` — browse the library (any org member)
- `POST /benchmark-tactics`, `PATCH /benchmark-tactics/:id`, `DELETE /benchmark-tactics/:id` (Admin only)
- `POST /benchmark-tactics/:id/load` — server-side equivalent of `createProjectFromBenchmark` in `benchmarkLibrary.ts`: creates a new `Project` (with server-assigned estimate number) from the tactic's current data
...
- **Data classification**: rate cards, benchmark tactics, and client-specific pricing are the most sensitive data in this system (competitive/commercial harm if leaked) — treat them as at least as sensitive as customer PII even though there's no traditional PII beyond preparer/business-manager names. Named-employee data from source benchmark spreadsheets must never reach the `BenchmarkTactic` table or the client (§2a) — enforce this at the import/admin-write layer, not just by convention.
